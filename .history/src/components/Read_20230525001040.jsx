...
import { showUser,deleteUser,favouriteUser } from '../store/slice/userDetail';
...
const Read = () => {
    ...
    console.log("the fave state", fav)

    const userData = useSelector((state)=>state.users.user)
    const searchData =useSelector((state)=> state.users.searchValue)
    const favData =useSelector((state)=> state.users.favoriteData)
    const dispatch = useDispatch()
    
    const handleFav=()=>[
        console.log("the favdata",favData)
    ...
    return (
       ...
       { showPop && <ViewUser id={id}  setShow={setShowPop}/> }
        ...
        <input className='form-check-input' name="favourite" value="userFav" onChange={(e)=>{setGenderData(e.target.value),setFav(true)}} type='radio'  checked={genderData==="userFav"}/>
        ...
        </div>
        {
             ...
             }) && (fav ===true?favData.map((elem)=>{return elem}):userData)
             ...
             .map((data)=>(
                ...
                <div className="card-body">
                    ...
                    <button className='card-link' onClick={()=> {dispatch(favouriteUser(data.id)),handleFav()}} >favourite</button>
                </div>
                </div>
               ))
        }
